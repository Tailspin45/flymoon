...
Queries OpenSky Network, ADSB-One (airplanes.live / api.adsb.one),
ADS-B Exchange (adsbexchange.com), and optionally a local RTL-SDR /
dump1090 / tar1090 receiver in parallel, then merges results by callsign
...
# ---------------------------------------------------------------------------
REQUEST_TIMEOUT: int = 5  # seconds per source
...
def fetch_multi_source_positions(
    ...
    with ThreadPoolExecutor(max_workers=4, thread_name_prefix="adsb-src") as pool:
