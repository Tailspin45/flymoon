...
from src.position import compute_track_velocity, predict_position, transit_corridor_bbox
...
def test_predict_position_due_north():
    ...
    assert (
        abs(actual_km - expected_km) < 0.5
    ), f"Distance should be ~{expected_km} km, got {actual_km:.2f} km"
    assert (
        abs(actual_bearing - 0.0) < 0.1 or abs(actual_bearing - 360.0) < 0.1
...
def test_predict_position_due_east():
    ...
    assert (
        abs(actual_km - expected_km) < 1.0
    ), f"Distance should be ~600 km, got {actual_km:.2f} km"
    assert (
        abs(actual_bearing - 90.0) < 0.5
...
def test_predict_position_zero_time():
    ...
    assert (
        abs(new_lat - lat0) < 1e-9 and abs(new_lon - lon0) < 1e-9
...
def test_compute_track_velocity_simple():
    ...
    assert (
        abs(speed - expected_speed) < 1.0
    ), f"Speed should be ~{expected_speed:.1f} km/h, got {speed:.1f}"
    assert (
        abs(heading) < 1.0 or abs(heading - 360) < 1.0
...
def test_compute_track_velocity_insufficient_data():
    ...
    assert compute_track_velocity(track) is None, "Single fix should return None"
...
def test_compute_track_velocity_same_timestamps():
    ...
    assert compute_track_velocity(track) is None, "Zero dt should return None"
...
def test_transit_corridor_bbox_contains_ground_point():
    ...
    assert (
        bbox.long_lower_left <= gp_lon <= bbox.long_upper_right
...
def test_transit_corridor_bbox_wider_at_low_altitude():
    ...
    print(
        f"✓ Bbox width: low alt={width_low:.2f}°, high alt={width_high:.2f}° — wider when lower"
    )
