...
from src.position import transit_corridor_bbox, get_my_pos
...
def snapshot(lat_ll, lon_ll, lat_ur, lon_ur):
    ...
    return {
        ...
        "p95_age_s": round(sorted(ages)[int(len(ages) * 0.95)], 1) if len(ages) >= 2 else None,
    }


def angular_error_from_staleness(stale_s: float, speed_kmh: float, dist_km: float, alt_km: float) -> float:
    """
...
def rate_limit_analysis(monitor_interval_min: int):
    ...
    has_creds = bool(
        os.getenv("OPENSKY_CLIENT_ID") or
...
def print_staleness_impact():
    ...
    print(f"  {'Staleness':>12}  {'Speed':>10}  {'Lateral err':>12}  {'Angular err':>12}")
    rows = []
    for stale_s in [5, 10, 15, 30, 60]:
        for speed_kmh in [600, 900]:
            ...
            print(f"  {stale_s:12.0f}s  {speed_kmh:10.0f}  {lat_km:12.3f} km  {ang:12.4f}°")
            rows.append({
                "stale_s": stale_s,
                "speed_kmh": speed_kmh,
...
def main():
    ...
    parser.add_argument("--observer-lat", type=float, default=float(os.getenv("OBSERVER_LATITUDE", "33.111369")))
    parser.add_argument("--observer-lon", type=float, default=float(os.getenv("OBSERVER_LONGITUDE", "-117.310169")))
    parser.add_argument("--observer-elev", type=float, default=float(os.getenv("OBSERVER_ELEVATION", "45")))
    parser.add_argument("--target", default="sun", choices=["sun", "moon"])
    parser.add_argument("--bbox", nargs=4, type=float, metavar=("LAT_LL", "LON_LL", "LAT_UR", "LON_UR"))
    parser.add_argument("--duration", type=int, default=0, help="Sampling duration in seconds (0=single snapshot)")
    parser.add_argument("--interval", type=int, default=60, help="Sampling interval in seconds (for --duration)")
    parser.add_argument("--output", default="docs/diag_logs/phase3_opensky_freshness.json")
    ...
    bbox_area_approx = (lat_ur - lat_ll) * (lon_ur - lon_ll) * 111.32**2 * math.cos(math.radians(obs_lat))
    ...
    if args.duration == 0:
        ...
        dt_str = datetime.fromtimestamp(snap["ts"], tz=timezone.utc).strftime("%H:%M:%S UTC")
        _info(f"Time: {dt_str}")
        _info(f"Aircraft in bbox: {snap['aircraft_count']} total, {snap['airborne_count']} airborne")
        if snap["ages_s"]:
            _info(f"Position age — min={snap['min_age_s']}s  mean={snap['mean_age_s']}s  max={snap['max_age_s']}s  p95={snap['p95_age_s']}s")
            ...
            _info(f"Typical staleness angular error (200km, 10km alt, 900km/h): {ang_err:.4f}°")
            if ang_err < 1.0:
                print(f"  PASS  OpenSky staleness acceptable — {ang_err:.4f}° < 1.0° HIGH threshold margin")
            else:
                print(f"  WARN  OpenSky staleness may cause misses — {ang_err:.4f}° is significant vs 2.0° HIGH threshold")
        else:
        ...
        while time.time() < end_time:
            ...
            _info(f"[{sample_count}] {dt_str}  n={snap['airborne_count']} airborne  ages: {ages_str}")
    ...
    output = {
        ...
        "bbox": {"lat_ll": lat_ll, "lon_ll": lon_ll, "lat_ur": lat_ur, "lon_ur": lon_ur},
