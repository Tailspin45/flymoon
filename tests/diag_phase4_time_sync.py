...
def run(host, port, output_path):
    ...
    try:
        ...
        results.append(_pass("pre_sync_equ_coord", f"ra={r.get('ra')} dec={r.get('dec')}"))
    except Exception as e:
    ...
    try:
        r = client._send_command(
            "pi_set_time",
            params=[{
                "year": now_utc.year, "mon": now_utc.month, "day": now_utc.day,
                "hour": now_utc.hour, "min": now_utc.minute, "sec": now_utc.second,
                "time_zone": "UTC",
            }],
            quiet=False,
            timeout_override=8,
        ...
        if r is not None:
            ...
            results.append(_warn("pi_set_time", "no response (may be normal for this firmware)"))
    except Exception as e:
    ...
    try:
        ...
        raw["post_sync_equ"] = r
        _info(f"RA={r.get('ra')}h  Dec={r.get('dec')}°")
        results.append(_pass("post_sync_equ_coord", f"ra={r.get('ra')} dec={r.get('dec')}"))
    except Exception as e:
    ...
    try:
        ...
        _info(f"Skyfield Sun: RA={sun['ra_h']:.4f}h  Dec={sun['dec_deg']:.4f}°  alt={sun['alt']:.2f}° az={sun['az']:.2f}°")
        ...
        if scope_ra is not None and scope_dec is not None:
            ...
            if delta_ra_deg < 5 and delta_dec < 5:
                results.append(_pass("scope_ra_vs_sun", f"Δra={delta_ra_deg:.2f}° Δdec={delta_dec:.2f}° — scope tracking Sun"))
            elif delta_ra_deg < 30:
                results.append(_warn("scope_ra_vs_sun", f"Δra={delta_ra_deg:.2f}° Δdec={delta_dec:.2f}° — moderate offset (stale?)"))
            else:
                results.append(_fail("scope_ra_vs_sun", f"Δra={delta_ra_deg:.2f}° Δdec={delta_dec:.2f}° — large offset (scope clock wrong?)"))
        else:
    ...
    try:
        r = client._send_command("scope_get_horiz_coord", quiet=False, timeout_override=5)
        raw["horiz_coord"] = r
        if r:
            _info(f"Response: {r}")
    ...
    try:
        ...
        _info(f"alt={t.get('alt')}° az={t.get('az')}°  (scope_alt={t.get('scope_alt')} scope_az={t.get('scope_az')})")
        ...
        results.append(_pass("telemetry", f"view_mode={t.get('view_mode')} firmware={t.get('firmware_ver')}"))
    except Exception as e:
