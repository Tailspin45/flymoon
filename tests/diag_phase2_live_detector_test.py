...
def feed_video_to_detector(
    ...
    return {
        ...
        "disk_found_at_s": (
            round(disk_found_at / ANALYSIS_FPS, 2) if disk_found_at else None
        ),
...
def main():
    ...
    for vpath_str in args.videos:
        ...
        r = feed_video_to_detector(
            vpath, sensitivity_scale=args.sensitivity, verbose=args.verbose
        )
        results.append(r)
        ...
        if r["disk_detected"]:
            print(
                f"    Disk  : ok  r={r['disk_radius']}px found @{r['disk_found_at_s']}s"
            )
        else:
        ...
        if r["detected"]:
            ...
            for d in r["detections"]:
                print(
                    f"      t={d['time_s']:.2f}s  CR={d['centre_ratio']:.2f}  "
                    f"conf={d['confidence']}  "
            ...
            if not r["disk_detected"]:
                print(
                    "    NOTE: disk not found — lunar contrast may be too low for Hough"
                )
                print(
                    "    TRY : --sensitivity 0.3, or check frame brightness in the video"
                )
    ...
    for r in results:
        ...
        print(
            f"  {r['file']:<33} {r.get('target','?'):<6} {disk:>5} "
