...
class SeestarClient:
    ...
    def start_lunar_mode(self) -> bool:
        ...
        try:
            try:
                ...
                logger.debug(f"start_lunar_mode: master claim failed (non-fatal): {_e}")
    ...
    def autofocus(self) -> dict:
        """Trigger autofocus and verify command acceptance when possible."""
        if not self._connected or not self.socket:
            raise RuntimeError("Not connected to Seestar")
        ...
        attempts = [
            ...
            ("start_auto_focuse", [], "verify_list"),
            # Some firmware/proxy stacks use corrected spelling.
            ("start_auto_focus", [], "verify_list_alt_method"),
            # Legacy fallback: top-level verify for no params.
            ("start_auto_focuse", None, "top_level_verify"),
            ("start_auto_focus", None, "top_level_verify_alt_method"),
        ]
        errors = []

        for method, params, verify_mode in attempts:
            try:
                ...
                logger.info(f"Autofocus accepted via {method} ({verify_mode})")
                return {
                    "sent": True,
                    "provider": "jsonrpc",
                    "confirmed": True,
                ...
                errors.append(f"{method}/{verify_mode}: {e}")
                logger.debug(
                    f"Autofocus attempt failed ({method}, {verify_mode}): {e}"
                )

        # Last resort: send fire-and-forget so firmware that suppresses RPC replies
        # can still run autofocus, but mark result as unconfirmed.
        try:
            self._send_command(
                "start_auto_focuse",
                params=[],
            ...
            logger.warning(
                "Autofocus command sent without RPC confirmation (no response)"
            )
            return {
                "sent": True,
                "provider": "jsonrpc",
                ...
                "verify_mode": "verify_list",
                "warning": "no_rpc_ack",
                "attempt_errors": errors[-3:],
            }
        except Exception as e:
            errors.append(f"start_auto_focuse/verify_list/fire_and_forget: {e}")
            raise RuntimeError(
                "Autofocus command failed; last attempts: " + " | ".join(errors[-4:])
            )
